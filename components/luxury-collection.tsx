...
export default function LuxuryCollection() {
  ...
  const [activeTab, setActiveTab] = useState("medellin") // Default active destination
  ...
  const handleTabChange = (tab) => {
    if (tab !== activeTab) {
      ...
      setTimeout(() => {
        setActiveTab(tab)
        setCurrentImageIndex(0) // Reset image index when changing tabs
        setTimeout(() => {
    ...
    {Object.keys(destinations).map((key) => (
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {/* Main image with carousel - 2 columns */}
          <div className="lg:col-span-2 relative rounded-2xl overflow-hidden h-64 md:h-72 lg:h-auto shadow-xl group">
            {/* Carousel Image */}
          ...
          {/* Description & Activities - 2 columns */}
...
<section className="py-8 bg-gray-50 dark:bg-gray-950">
  <div className="container mx-auto px-4">
    {/* User Experience Grid Component */}
    <UserExperienceGrid />
  </div>
</section>
